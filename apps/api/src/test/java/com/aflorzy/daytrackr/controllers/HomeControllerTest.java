...
class HomeControllerTest {
  ...
  void tokenWithBasicThenGetToken() throws Exception {
    MvcResult result = this.mvc.perform(post("/token").with(httpBasic("aflorzy", "password")))
  ...
  public void rootWithBasicStatusIsUnauthorized() throws Exception {
    this.mvc.perform(get("/").with(httpBasic("aflorzy", "password"))).andExpect(status().isUnauthorized());
  }
